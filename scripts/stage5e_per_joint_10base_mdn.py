...
class PerJointRotationMDN(nn.Module):
    """J joints, each with one-shot MDN + cross-attention + learned temporal smoother."""
    def __init__(self, n_joints=21, n_timesteps=96, n_components=4, feature_dim=32, hidden_dim=128):
        ...
        # Input: (B, J*K*3, T) → groups=J → each joint's K*3 channels filtered independently
...
def train_model(model, data, bases_rot, n_epochs=200, batch_size=16, lr=1e-3, device="cpu"):
    ...
    for j in range(n_joints):
        bj = torch.tensor(bases_rot[j], dtype=torch.float32, device=device)
    ...
    for epoch in range(1, n_epochs + 1):
        ...
        for st in range(0, len(tr_idx), batch_size):
            ...
            base_b = X_tr[b, :, 6:12]; act_b = actions[b_idx]; tb = Y_tr[b]
        ...
        with torch.no_grad():
            v_logits, v_mu, v_sigma = model(
                X_val[:, :, :3], X_val[:, :, 3:6], val_act, X_val[:, :, 6:12], apply_smooth=True)
...
def generate_motion(model, bases_rot, start_rot, goal_rot, trans,
    ...
    for j in range(n_joints):
        ...
        base_f_np[j, 0] = length; base_f_np[j, 1] = curv; base_f_np[j, 2] = dev
        base_f_np[j, 3] = bj[:, 2].max(); base_f_np[j, 4] = bj[:, 2].std()
        base_f_np[j, 5] = bj[-1, 2] - bj[0, 2]
    
    x_raw = np.concatenate([start_np.reshape(1, n_joints, 3),
                            goal_np.reshape(1, n_joints, 3),
                            base_f_np.reshape(1, n_joints, 6)], axis=-1).astype(np.float32)
    ...
    with torch.no_grad():
        ...
        logits, mu_std, log_sigma = model(x_t[:, :, :3], x_t[:, :, 3:6], act_t, x_t[:, :, 6:12], apply_smooth=True)
