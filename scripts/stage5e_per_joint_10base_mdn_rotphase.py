...
class PerJointRotationMDN(nn.Module):
    """J joints, each with one-shot MDN + cross-attention + phase skip + temporal smoother."""
    def __init__(self, n_joints=21, n_timesteps=96, n_components=4, feature_dim=32, hidden_dim=128):
        super().__init__()
        self.n_joints = n_joints
        self.n_timesteps = n_timesteps

        # Encoder: geometry only (start+goal+shape+action = 15-d). Phase goes via skip.
        ...
        # Per-joint learnable smoother: each joint has its own Conv1d filter (groups=J)
        smooth_in = n_joints * 12  # J * K * 3 = 24 * 4 * 3 = 288
        self.smooth = nn.Conv1d(smooth_in, smooth_in, kernel_size=3,
                                padding=1, groups=n_joints, bias=False)
        nn.init.dirac_(self.smooth.weight)  # start as identity

    def forward(self, start_rot, goal_rot, action, base_features, apply_smooth=False):
        B, J = start_rot.shape[:2]
        K = self.mdns[0].logits.out_features  # n_components
        T = self.n_timesteps
        action_exp = action.unsqueeze(1).expand(B, J, 3)
        # base_features: (B, J, 14) = shape(6) + phase(8)
        base_shape = base_features[:, :, :6]    # geometry
        base_phase = base_features[:, :, 6:14]  # timing — RAW skip to MDN
        # Encoder: geometry only (15-d). Phase bypasses encoder → pure timing signal.
        feat_geo = torch.cat([start_rot, goal_rot, base_shape, action_exp], dim=-1)
        encoded = self.joint_encoder(feat_geo)   # (B, J, 32)
        context, _ = self.attn(encoded, encoded, encoded)  # (B, J, 32)
        # MDN input: encoded + context + raw phase = time-aware
...
def train_model(model, data, bases_rot, n_epochs=200, batch_size=16, lr=1e-3, device="cpu"):
    ...
    # Only standardize start+goal (6 dims that vary per sample). Base features
    # (shape+phase) are constant across samples → std≈0 → would explode.
    GEO_DIM = 6
    ...
    base_raw = X_input.numpy().reshape(-1, 20)[:, GEO_DIM:]
...
def generate_motion(model, bases_rot, start_rot, goal_rot, trans,
    ...
    GEO_DIM = 6  # start+goal only (rest are constant base features)
    if x_scaler is not None:
        x_flat = x_raw.reshape(-1, 20)
        geo_std = x_scaler.transform(x_flat[:, :GEO_DIM])
        base_raw = x_flat[:, GEO_DIM:]
