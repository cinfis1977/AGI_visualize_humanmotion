...
class OneShotJointMDN(nn.Module):
    """Per-joint MDN with compressed global time injected into giant Linear."""
    def __init__(self, feature_dim, n_timesteps=96, n_components=4, hidden_dim=128, n_hidden=3,
        ...
        # Compact global time: raw sin/cos (T,2) → small MLP → (time_compact,) → concat to trunk
        t_norm = torch.linspace(0.0, 1.0, n_timesteps).unsqueeze(1)  # (T, 1)
        time_2d = torch.cat([torch.sin(2*torch.pi*t_norm), torch.cos(2*torch.pi*t_norm)], dim=-1)
        self.register_buffer('time_raw', time_2d)  # (T, 2)
        self.time_compressor = nn.Sequential(
            nn.Linear(n_timesteps * 2, hidden_dim), nn.GELU(),
            nn.Linear(hidden_dim, time_compact),
        )

        # Giant Linear: (hidden + time_compact) → K*T*3 — original architecture preserved
        self.mu = nn.Linear(hidden_dim + time_compact, n_components * self.target_dim)
    ...
    def forward(self, x):
        ...
        # Compress global time: (T,2) → flatten → MLP → (time_compact,)
        t_compressed = self.time_compressor(self.time_raw.reshape(-1).unsqueeze(0))  # (1, time_compact)
        t_exp = t_compressed.expand(B, -1)  # (B, time_compact)
        h_aug = torch.cat([h, t_exp], dim=-1)  # (B, hidden + time_compact)
        mu = self.mu(h_aug).view(B, K, self.target_dim)

        return self.logits(h), mu, self.log_sigma(h).clamp(-4.0, 3.0)
