#!/usr/bin/env python3
"""
Stage5E-fixed — stage5e + amplitude preservation fixes.

Fixes applied (per analysis):
  1. NO-SCALE residuals — predict raw radian residuals (eliminates scaler amplitude trap)
  2. FK 3D position loss — forces foot-ground contact via Cartesian penalty
  3. Per-joint learned smoother — Conv1d with dirac init on residual output
  4. Boundary-locked residuals — start/goal exact via projection
  5. Safe-improvement — don't make rotation error worse than baseline
"""
...
from smpl_fk_torch import fk_smpl_torch  # differentiable FK
Array = np.ndarray
...
def train_model(model, data75, bases, epochs=200, bs=16, lr=1e-3, device="cpu",
    ...
    print(f"  stage5e-fixed: no-scale residuals + FK loss; train={len(ti)} val={nv}")
    ...
    for ep in range(1, epochs+1):
        ...
        for st in range(0, len(ti), bs):
            ...
            win = err.argmin(-1)  # (B,J)
            # Manual winner-take-most (DirectML-safe: scatter_ unreliable on DML)
            w = torch.full_like(err, 0.05/(K-1))
            for b_idx in range(win.shape[0]):
                for j_idx in range(J):
                    w[b_idx, j_idx, win[b_idx, j_idx]] = 0.95
            recon = (w*err).sum(-1).mean()
            gate = sum(F.cross_entropy(logits[:,j], win[:,j]) for j in range(J))/J

            # FK 3D position loss — DIFFERENTIABLE (gradients flow to rotations)
            # Per-joint weights: feet/ankles get higher weight (lever-arm compensation)
            FK_WEIGHTS = torch.ones(J, device=device)
            FK_WEIGHTS[7] = 8.0   # L_ankle
            FK_WEIGHTS[8] = 8.0   # R_ankle
            FK_WEIGHTS[10] = 10.0 # L_foot
            FK_WEIGHTS[11] = 10.0 # R_foot
            FK_WEIGHTS[4] = 3.0   # L_knee
            FK_WEIGHTS[5] = 3.0   # R_knee

            # Winner residual (keep on GPU for autograd)
            mu_win = mu[0].gather(1, win[0].view(J,1,1,1).expand(J,1,T,3)).squeeze(1)  # (J, T, 3)
            pred_rot = BL[b[0]] + mu_win.permute(1, 0, 2)  # (T, J, 3) — differentiable!
            true_rot = R[b[0]]  # (T, J, 3) — no permute!
            trans_t = Tr[b[0]]  # (T, 3)

            # Differentiable FK: position error in world space
            pred_flat = pred_rot.reshape(T, 72)
            true_flat = true_rot.reshape(T, 72)
            pred_pos = fk_smpl_torch(pred_flat[:, 3:66].unsqueeze(0),
                                     pred_flat[:, :3].unsqueeze(0),
                                     trans_t.unsqueeze(0)).squeeze(0)  # (T, 24, 3)
            true_pos = fk_smpl_torch(true_flat[:, 3:66].unsqueeze(0),
                                     true_flat[:, :3].unsqueeze(0),
                                     trans_t.unsqueeze(0)).squeeze(0)

            # Weighted position MSE per joint
            pos_err = ((pred_pos - true_pos)**2).sum(-1)  # (T, 24)
            fk_loss = (pos_err * FK_WEIGHTS.unsqueeze(0)).mean()

            # --- Amplitude & dynamics auxiliary losses (numpy, detached) ---
            with torch.no_grad():
                pred_rot_np = pred_rot.detach().cpu().numpy()  # (T, J, 3)
                true_rot_np = true_rot.detach().cpu().numpy()
                pred_std = np.std(pred_rot_np, axis=0)  # (J, 3)
                true_std = np.std(true_rot_np, axis=0)
                amp_loss = torch.tensor(np.mean((pred_std - true_std)**2), dtype=torch.float32, device=device)
                pred_vel = pred_rot_np[1:] - pred_rot_np[:-1]
        ...
        with torch.no_grad():
            ...
            # Manual winner-take-most (DirectML-safe)
            for b_idx in range(vw.shape[0]):
                for j_idx in range(J):
                    vw2[b_idx, j_idx, vw[b_idx, j_idx]] = 0.95
        ...
        if ep==1 or ep%10==0 or ep==epochs:
            print(f"  ep {ep:4d}: train={total/max(cnt,1):.4f} val={val_loss:.6f} "
                  f"fk={fk_loss.item():.6f} amp={amp_loss.item():.6f} best={best_val:.6f}")
...
def main():
    ...
    # Resolve device (support DirectML)
    device = args.device
    if device == "dml":
        import torch_directml
        device = torch_directml.device()
        print(f"[Stage5E-v3] Using DirectML GPU: {device}")
    else:
        print(f"[Stage5E-v3] Using device: {device}")
