...
def train_model(model, data, bases_rot, n_epochs=200, batch_size=16, lr=1e-3, device="cpu"):
    ...
    INPUT_DIM = 20  # start(3) + goal(3) + base_features(14)
    x_scaler = StandardScalerNP().fit(X_input.numpy().reshape(-1, INPUT_DIM))
    X_std = torch.tensor(x_scaler.transform(X_input.numpy().reshape(-1, INPUT_DIM)).reshape(n_samples, n_joints, INPUT_DIM),
                         dtype=torch.float32, device=device)

    # Split 15% validation (baseball-style)
    n_val = max(1, int(0.15 * n_samples))
    val_idx = torch.randperm(n_samples)[:n_val]
    tr_idx = torch.randperm(n_samples)[n_val:]
...
def generate_motion(model, bases_rot, start_rot, goal_rot, trans,
    ...
    if x_scaler is not None:
        x_std = x_scaler.transform(x_raw.reshape(-1, INPUT_DIM)).reshape(1, n_joints, INPUT_DIM)
    else:
        x_std = x_raw
